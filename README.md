...
## NEW MAJOR RELEASE in early 2019 with BREAKING CHANGES

New release in early 2019 will include the following major enhancements ([litehelpers/Cordova-sqlite-storage#773](https://github.com/litehelpers/Cordova-sqlite-storage/issues/773)):
...
- ~~error `code` will always be `0` (which is already the case on Windows); actual SQLite3 error code will be part of the error `message` member whenever possible ([litehelpers/Cordova-sqlite-storage#821](https://github.com/litehelpers/Cordova-sqlite-storage/issues/821))~~ (not expected in early 2019)
...
<!-- XXX FUTURE TBD: Android Circle-CI/iOS Travis-CI WANTED working again for this plugin version -->
...
- This version is **not** supported by PhoneGap Developer App or PhoneGap Desktop App.
...
- _This plugin version uses a `before_plugin_install` hook to install sqlite3 library dependencies from `cordova-sqlite-ext-deps` via npm._
- This _plugin version_ includes the following XXX ext-XXX features XXX not normally supported on XXX by XXX:
...
- This plugin version includes the following extra features: BASE 64 (all platforms Android/iOS/macOS/Windows), REGEXP (Android/iOS/macOS)
...
- XXX THIS PLUGIN VERSION now supports SELECT BLOB data in Base64 format on all platforms in addition to REGEXP (Android/iOS/macOS) and pre-populated database (all platforms).
